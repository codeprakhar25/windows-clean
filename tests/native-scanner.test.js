...
(async () => {
  ...
  assert(rustMain.includes("fn delete_browser_cache_target_at(root: &Path, now: SystemTime)"), "native browser cache executor should expose a clock-injected deleter for proof tests");
  assert(rustMain.includes("delete_browser_cache_target_at(root, SystemTime::now())"), "production browser cache executor should use the real clock for age gating");
  assert(rustMain.includes("fn delete_single_browser_cache_file_at("), "native browser cache executor should age-gate deletion through a clock-injected file helper");
  assert(rustMain.includes("browser_cache_deleter_removes_only_old_cache_files"), "native browser cache executor should have a deletion proof test");
  assert(rustMain.includes("fn delete_gradle_cache_target_at(root: &Path, now: SystemTime)"), "native Gradle executor should expose a clock-injected deleter for proof tests");
  assert(rustMain.includes("delete_gradle_cache_target_at(root, SystemTime::now())"), "production Gradle executor should use the real clock for age gating");
  assert(rustMain.includes("fn delete_single_gradle_cache_file_at("), "native Gradle executor should age-gate deletion through a clock-injected file helper");
  assert(rustMain.includes("gradle_cache_deleter_removes_only_old_cache_files"), "native Gradle executor should have a deletion proof test");
  assert(rustMain.includes("fn delete_user_cache_target_at(root: &Path, now: SystemTime)"), "native user cache executor should expose a clock-injected deleter for proof tests");
  assert(rustMain.includes("delete_user_cache_target_at(root, SystemTime::now())"), "production user cache executor should use the real clock for age gating");
  assert(rustMain.includes("fn delete_single_user_cache_file_at("), "native user cache executor should age-gate deletion through a clock-injected file helper");
  assert(rustMain.includes("user_cache_deleter_removes_only_old_non_metadata_files"), "native user cache executor should have a deletion proof test");
  assert(rustMain.includes("fn delete_android_cache_target_at(root: &Path, now: SystemTime)"), "native Android cache executor should expose a clock-injected deleter for proof tests");
  assert(rustMain.includes("delete_android_cache_target_at(root, SystemTime::now())"), "production Android cache executor should use the real clock for age gating");
  assert(rustMain.includes("fn delete_single_android_cache_file_at("), "native Android cache executor should age-gate deletion through a clock-injected file helper");
  assert(rustMain.includes("android_cache_deleter_removes_only_old_cache_files"), "native Android cache executor should have a deletion proof test");
  assert(rustMain.includes("fn delete_shader_cache_target_at(root: &Path, now: SystemTime)"), "native shader cache executor should expose a clock-injected deleter for proof tests");
  assert(rustMain.includes("delete_shader_cache_target_at(root, SystemTime::now())"), "production shader cache executor should use the real clock for age gating");
  assert(rustMain.includes("fn delete_single_shader_cache_file_at("), "native shader cache executor should age-gate deletion through a clock-injected file helper");
  assert(rustMain.includes("shader_cache_deleter_removes_only_old_cache_files"), "native shader cache executor should have a deletion proof test");
  assert(rustMain.includes("fn delete_pip_cache_target_at(root: &Path, now: SystemTime)"), "native pip cache executor should expose a clock-injected deleter for proof tests");
  assert(rustMain.includes("delete_pip_cache_target_at(root, SystemTime::now())"), "production pip cache executor should use the real clock for age gating");
  assert(rustMain.includes("fn delete_single_pip_cache_file_at("), "native pip cache executor should age-gate deletion through a clock-injected file helper");
  assert(rustMain.includes("pip_cache_deleter_removes_only_old_cache_files"), "native pip cache executor should have a deletion proof test");
  assert(rustMain.includes("fn downloads_cleanup_target_reject_code_at(value: &str, now: SystemTime)"), "native Downloads executor should expose a clock-injected target validator for proof tests");
  assert(rustMain.includes("downloads_cleanup_target_reject_code_at(value, SystemTime::now())"), "production Downloads executor should use the real clock for target age checks");
  assert(rustMain.includes("fn path_age_days_at(path: &Path, now: SystemTime)"), "native age checks should expose a clock-injected path age helper");
  assert(rustMain.includes("downloads_target_validation_accepts_only_old_reviewed_installer_files"), "native Downloads executor should have a target validation proof test");
  assert(rustMain.includes("fn large_file_archive_target_reject_code_at("), "native large-file archive executor should expose a clock-injected target validator for proof tests");
  assert(rustMain.includes("large_file_archive_target_reject_code_at(value, archive_destination, SystemTime::now())"), "production large-file archive executor should use the real clock for target age checks");
  assert(rustMain.includes("large_file_archive_target_validation_accepts_only_old_large_reviewed_files"), "native large-file archive executor should have a target validation proof test");
  assert(rustMain.includes("project_dependency_target_validation_rejects_forbidden_locations"), "native project dependency executor should have a target validation proof test");
  assert(rustMain.includes("project_dependency_deleter_removes_only_reviewed_node_modules"), "native project dependency executor should have a deletion proof test");
  assert(rustMain.includes("write_target_forbidden(\"item-review-project-cache\", &original)"), "native project dependency validator should enforce forbidden route policy on original targets");
  assert(rustMain.includes("write_target_allowed(\"item-review-project-cache\", &original)"), "native project dependency validator should enforce allowlisted route policy on original targets");
        ...
        items: [
          {
            id: "old-temp-file",
            name: "old-temp.tmp",
            path: "C:\\Users\\real\\AppData\\Local\\Temp\\spaceguard\\old-temp.tmp",
            bytes: 5 * MB,
            age_days: 3,
            kind: "temp file",
  ...
  assert(!Object.prototype.hasOwnProperty.call(directRouteCapability, "firstRouteProof"), "native direct route normalization must not synthesize proof ceremony");
  assert.deepStrictEqual(
