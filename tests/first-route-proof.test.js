...
(async () => {
  ...
  assert.strictEqual(npmPacket.status, "blocked", "non-fixture routes should stay blocked until first-route proof is accepted");
  assert.strictEqual(npmPacket.routeValidation.status, "first-route-proof-required", "non-fixture routes should preserve the first-route proof gate");
  assert.strictEqual(npmPacket.firstRouteProof.required, true, "non-fixture route packet should expose first-route proof requirement");
  assert(npmPacket.nextSteps.some((step) => step.includes("SPACEGUARD_FIRST_ROUTE_COMPLETION_CHECK")), "non-fixture route packet should point to first-route completion env var");
