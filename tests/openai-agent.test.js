...
(async () => {
  ...
  const manualContextJson = JSON.stringify(manualContext);
  assert(!manualContextJson.includes("C:\\Users\\real"), "OpenAI context should redact concrete user profile paths");
  assert(!manualContextJson.includes("C:\\\\Users\\\\real"), "OpenAI context should redact JSON-escaped concrete user profile paths");
  assert(manualContext.projectDependencyReviewTargets[0].path.startsWith("%USERPROFILE%\\"), "OpenAI project dependency paths should preserve shape without the username");
  assert(manualContext.userCacheTargets[0].path.startsWith("%USERPROFILE%\\"), "OpenAI cache paths should preserve shape without the username");
