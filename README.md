...
Set `OPENAI_API_KEY` in `.env` before using the OpenAI cleanup advisor. `npm run route:arm -- --route npm-cache` enables the selected cleanup type and turns every other cleanup route flag off.
