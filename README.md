...
That verifier now requires the captured `selected-route-setup.json`, the `resolve-selected-route` command record, the ready `setup-route.json`, the selected-route validation packet, and any route-specific setup requirements to agree on the same canonical route and scoped executor flag.
